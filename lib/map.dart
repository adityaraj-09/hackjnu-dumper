...
import 'package:hackjnu_dumper/database/maps_api.dart';
...
class MapScreen extends StatefulWidget {
  var dumping = false;
  MapScreen({super.key, required this.dumping});
...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  Future<bool> getCoordniates() async {
    var data = Provider.of<MapProvider>(context, listen: false).selectedShovel;
    Uri uri = getRouteUrl("${data["dumperLoc"][0]},${data["dumperLoc"][1]}",
        "${data["location"][0]},${data["location"][1]}");
    if (widget.dumping) {
      uri = getRouteUrl(
          "${data["location"][0]},${data["location"][1]}", "77.0628, 28.4357");
    }
    ...
    getCoordniates().then((value) => {
          timer = Timer.periodic(const Duration(milliseconds: 500), (timer) {
            ...
            if (idx == points.length - 1) {
              socketMethods.sendData(points[idx].longitude,
                  points[idx].latitude, "njbjbjka", true, distance, time);
            } else {
              socketMethods.sendData(points[idx].longitude,
                  points[idx].latitude, "njbjbjka", false, distance, time);
            }
            setState(() {});
                ...
                MarkerLayer(
                  markers: [
                    Marker(
                      width: 45.0,
                      height: 45.0,
                      point: LatLng(28.4357,
                          77.0628), // Example landmark coordinates (San Francisco)
                      child: Container(
                    ...
                    Marker(
                      ...
                      point: LatLng(
                          shovel["location"][1],
                          shovel["location"][
                              0]), // Example landmark coordinates (San Francisco)
                      child: Container(
                        decoration: BoxDecoration(
                            image: DecorationImage(
                                image: AssetImage(
